...
class ContactsService {
    ...
    async createContact(userId, input) {
        ...
        if (existing)
            throw { statusCode: 409, message: 'Contact with this phone already exists' };
    ...
    async updateContact(userId, contactId, input) {
    ...
    async optOutContact(userId, contactId, optOut) {
        const existing = await this.prisma.contact.findFirst({ where: { id: contactId, userId } });
        if (!existing)
            throw { statusCode: 404, message: 'Contact not found' };
        const updated = await this.prisma.contact.update({
    ...
    async bulkTagContacts(userId, contactIds, tags, action) {
    ...
    async importContacts(userId, csvData) {
        const lines = csvData.trim().split('\n');
        if (lines.length < 2)
            throw { statusCode: 400, message: 'CSV has no data rows' };
        ...
        if (phoneIdx === -1)
            throw { statusCode: 400, message: 'CSV must have a phone column' };
        let created = 0;
        let skipped = 0;
